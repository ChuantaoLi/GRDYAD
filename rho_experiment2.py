    ...
    for i in range(n):
        for j in range(n):
            if y[i] == y[j]:
                ...
                G[i, j] = np.exp(-dist / para)  # Gaussian kernel to calculate similarity
    D = np.diag(G.sum(axis=1))
    L = D - G  # Laplacian matrix
    return G, L


def graph_regularized_projection(X, L, gamma):
...
def ada_boost_train_dynamic(X, y, num_iter, random_state, para):
    ...
    for i in range(1, num_iter + 1):
        ...
        P = graph_regularized_projection(X_res, L, 0.01)
        X_proj = X_res @ P
        X_full_proj = X @ P
...
if __name__ == '__main__':
    ...
    for para in [1.1, 1.2, 1.3, 1.4, 1.5, 2.0]:
        print('para:', para)
        print('*' * 40)
        ...
        data_folder = 'Experiment2/yeast3-5-fold'

        for i in range(1, 6):
            train_file = os.path.join(data_folder, f'yeast3-5-{i}tra.dat')
            test_file = os.path.join(data_folder, f'yeast3-5-{i}tst.dat')

            if not os.path.exists(train_file) or not os.path.exists(test_file):
                # print(f"Warning: Missing file for fold {i} (train: {train_file}, test: {test_file})")
                continue

            X_train, y_train = load_keel_dat(train_file)
            X_test, y_test = load_keel_dat(test_file)

            results = run_repeated_holdout_have_train_test(X_train, y_train, X_test, y_test, para)
            ggmean.append(results['gmean'][0])
            aauc.append(results['auc'][0])

        print(f'avg_gmean: {np.mean(ggmean):.3f}±{np.std(ggmean):.3f}')
        print(f'avg_auc: {np.mean(aauc):.3f}±{np.std(aauc):.3f}')
